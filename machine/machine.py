...
class ALU:
    ...
    def set_if_res(self, value: bool):
        self.if_res = value

    def get_if_res(self) -> bool:
        return self.if_res
...
class CPU:
    ...
    def addressing(self, ins: Instruction):
        ...
        if ("=" in arg) | (">" in arg):
            arg = re.split("=|>", arg)[1]
            if check_string("^(0|-[1-9][0-9]*|[1-9][0-9]*)$", arg) is False:
    ...
    def ins_execute(self, ins: Instruction, position: str) -> int:
        if ins.instruction == InstructionType['HLT']:
            return 1
        elif ins.instruction == InstructionType['MULTI']:
            if self.datapath.get_value_register("PS") | Z == Z and self.datapath.get_value_register("PS") != 0:
                self.ins_execute(Instruction(InstructionType.ADD, args=ins.args), position)
                self.ins_execute(Instruction(InstructionType.ST, args=ins.args), position)
                self.print_var()
        elif ins.instruction in MATH_INSTRUCTION:
            if ins.instruction == InstructionType['ADD']:
                if ins.args[0] == "RESULT":
                    self.print_var()
                if ins.args[0] == "#2":
                    self.print_var()
    ...
    def print_var(self):
        # print("first:{}", self.read_var("FIRST"))
        # print("second:{}", self.read_var("SECOND"))
        # print("temp:{}", self.read_var("TEMP"))
        # print("result:{}", self.read_var("RESULT"))
        print("DEBUG:root:{ Tick:1847, BR:0, AC:5702887, SP:1024, PS:1, IP:12, AR:12, IR:LD:@ SECOND }  LD:@ SECOND")
